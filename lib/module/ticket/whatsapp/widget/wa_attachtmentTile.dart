...
class WaAttachmentTile extends StatelessWidget {
  final WhatsAppMessage? waUserConversation;

  const WaAttachmentTile({super.key, this.waUserConversation});

  @override
  Widget build(BuildContext context) {
    if (waUserConversation?.mediaUrl == null ||
        waUserConversation?.mediaUrl?.isEmpty == true) {
      return const SizedBox.shrink();
    }

    // Check attachment type
    if (waUserConversation?.type == "image") {
      return _buildImageAttachment(context);
    } else if (waUserConversation?.type == "audio" ||
        _isAudioFile(waUserConversation?.localUrl)) {
      return _buildAudioAttachment(context);
    } else if (waUserConversation?.type == "video" ||
        _isVideoFile(waUserConversation?.localUrl)) {
      return _buildVideoAttachment(context);
    } else if (waUserConversation?.localUrl != null &&
        waUserConversation?.type != "text") {
  ...
  bool _isAudioFile(String? url) {
    ...
    return ['mp3', 'mpeg', 'wav', 'ogg', 'm4a', 'aac'].contains(extension);
  }
  ...
  Widget _buildImageAttachment(BuildContext context) {
    ...
    final imgUrl = ApiUrls.whatsAppBaseUrl;
    print("wa image show:${imgUrl + waUserConversation!.localUrl.toString()}");
    return Stack(
      ...
      children: [
        GestureDetector(
            ...
            Navigator.push(
              ...
              MaterialPageRoute(
                builder: (context) => FullScreenImageView(
                  imageUrl: imgUrl + waUserConversation!.localUrl.toString(),
                  localFilePath:
                      imgUrl + waUserConversation!.localUrl.toString(),
                  heroTag: heroTag,
                ),
              ),
            );
          ...
          child: Hero(
              tag: heroTag,
              child: (waUserConversation?.mediaUrl != null &&
                      waUserConversation!.mediaUrl!.isNotEmpty)
                  ? Image.network(
                      imgUrl + waUserConversation!.localUrl.toString(),
        ...
        Positioned(
          ...
          child: GestureDetector(
            onTap: () {
              if (waUserConversation?.localUrl != null) {
                _downloadFile(
                  context,
                  imgUrl + waUserConversation!.localUrl.toString(),
                );
  ...
  Widget _buildAudioAttachment(BuildContext context) {
    final waAudioUrl =
        ApiUrls.whatsAppBaseUrl + waUserConversation!.localUrl.toString();
    print("waAudio:${waAudioUrl}");
  ...
  Widget _buildFileAttachment(BuildContext context) {
    ...
    return InkWell(
      onTap: () => _downloadFile(
          context, ApiUrls.whatsAppBaseUrl + waUserConversation!.localUrl!),
      child: Container(
        ...
        child: Row(
          children: [
            Icon(_getFileIcon(waUserConversation?.localUrl), size: 24.sp),
            SizedBox(width: 8.w),
            Flexible(
              child: Text(
                "Download $fileName",
